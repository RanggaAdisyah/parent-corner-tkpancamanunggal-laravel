...
<head>
    ...
    <link rel="stylesheet" href="{{ url('/css/style/orang_tua/hubungi_guru.css') }}">
</head>
<body>
    <div class="dashboard-guru">
        {{-- Sidebar Orang Tua --}}
        @include('partials.sidebar_orang_tua', ['active' => 'hubungi-guru'])
                ...
                <section class="teacher-card">
                    ...
                    <div class="teacher-card-body">
                        <div class="teacher-avatar-wrap">
                            @if($guru && $guru->user && $guru->user->foto)
                                <img src="{{ url('storage/' . $guru->user->foto) }}" alt="{{ $guru->user->name }}" class="teacher-avatar" style="object-fit: cover;">
                            @else
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($guru->user->name ?? 'Guru') }}&background=e2e8f0&color=475569" alt="Avatar" class="teacher-avatar">
                            @endif
                            <span class="teacher-online" aria-label="Online"></span>
                        </div>

                        <h2 class="teacher-name">{{ $guru ? $guru->user->name : 'Belum Ditentukan' }}</h2>
                        ...
                        <div class="teacher-role-tag">
                            ...
                            Wali Kelas {{ $siswa && $siswa->kelasLokal ? $siswa->kelasLokal->nama_kelas : '-' }}
                        </div>

                        <p class="teacher-bio">
                            @if($guru)
                                Selamat datang, Ayah & Bunda. Saya {{ $guru->user->name }}, siap berdiskusi mengenai perkembangan, potensi, dan keseharian ananda selama di sekolah.
                            @else
                                Wali kelas untuk ananda belum ditentukan. Silakan hubungi admin sekolah.
                            @endif
                        </p>
                        ...
                        <div class="teacher-actions">
                            @if($guru && $guru->no_wa)
                            @php
                                $wa = preg_replace('/^0/', '62', $guru->no_wa);
                            @endphp
                            <a href="https://wa.me/{{ $wa }}" target="_blank" class="btn-whatsapp">
                            ...
                            <a href="https://mail.google.com/mail/?view=cm&fs=1&to={{ $guru->user->email }}" target="_blank" class="btn-email">
